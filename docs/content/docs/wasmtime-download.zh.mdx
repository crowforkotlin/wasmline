...
`version` 是可执行文件报告并用于 AOT 兼容性校验的三段版本；`releaseVersion` 是提供二进制资产的四段下游 GitHub Release 版本。`directory` 可用于指定精简 Wasmtime 的缓存目录或可执行文件目录。未设置时，插件依次检查 `WASMTIME_ROOT` 和 `~/.wasmline/wasmtime`。
