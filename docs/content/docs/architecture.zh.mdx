...
Component Model 调用通过 Wasmtime Component Model API 加载完整且已经编译完成的 component binary。该路径使用 Component Model typed value，不要求 Core 线性内存、`__wasmline_wasi_entry` 或 Core 响应帧。Wasmline 不编译 WIT、WIT-Kotlin，也不运行 `wit-bindgen` 或生成 component adapter。可选的 `contractMetadata` 用于记录调用契约、执行校验和生成文档，不是编译器输入。
