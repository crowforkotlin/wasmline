...
|---|---|---|---|
| `CORE_WASM` | `WASMLINE_CORE_V1` | `Session`、action 路由和线性内存 | 使用 |
...
`WASMLINE_CORE_V1` 是 Core Wasmline 模块的服务协议。其响应字节使用固定的 `WLMF` magic 标记和一个字节的 `frameVersion`，当前值为 `1`。magic 用于识别响应格式，不提供身份认证。`frameVersion` 表示响应字节布局，不表示 Wasmtime、Kotlin、框架或业务 API 版本。读取方只有在该字段为 `1` 时才按 V1 布局解析。
