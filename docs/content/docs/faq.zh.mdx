...
对于使用 action 名称和字节 payload 的 Wasmline 服务模块，使用 `CORE_WASM` 与 `WASMLINE_CORE_V1`。对于具有明确数值 export ABI 的 Core Wasm 模块，使用 `CORE_WASM` 与 `RAW_EXPORT`。对于完整的 component binary 和 Component Model typed value，使用 `COMPONENT_MODEL` 与 `COMPONENT_EXPORT`。
