...
Component Model 路径的输入是已经编译完成的 component binary。Wasmline 不编译 WIT 或 WIT-Kotlin，不运行 `wit-bindgen`，也不生成 component adapter。可选的 `contractMetadata` 用于描述 export 契约，不是 WIT 编译器的输入。
