...
The Component Model path starts with an already compiled component binary. Wasmline does not compile WIT or WIT-Kotlin, run `wit-bindgen`, or generate component adapters. Optional `contractMetadata` describes the export contract; it is not an input to a WIT compiler.
