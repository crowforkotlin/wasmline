...
Wasmline exposes a single API from `commonMain` / `hostMain` and selects an execution path per target platform. On native targets (Android, iOS, macOS, Linux, Windows), plugins execute through `wasmline-core` (C/C++, compiled with Zig 0.15.1) into the wasmtime C-API. On Web targets (Kotlin/JS, Kotlin/WasmJS), plugins execute through an inline JS runtime on the browser-native WebAssembly API — wasmtime is not involved.
