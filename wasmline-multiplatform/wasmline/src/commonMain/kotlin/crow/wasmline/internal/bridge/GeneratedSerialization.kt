...
@PublishedApi
internal inline fun <reified T> encodeGeneratedValue(factory: WasmlineSerializationFactory, value: T): ByteArray =
    factory.encode(serializer<T>(), value)

@PublishedApi
internal inline fun <reified T> decodeGeneratedValue(factory: WasmlineSerializationFactory, payload: ByteArray): T =
    factory.decode(serializer<T>(), payload)
 ...
 */
private class ArrayBackedSerializer(override val descriptor: SerialDescriptor, private val serializers: Array<KSerializer<*>>) :
 ...
 */
@PublishedApi
internal fun buildParamsDescriptor(name: String, serializers: Array<KSerializer<*>>): SerialDescriptor = buildClassSerialDescriptor(name) {
    for (i in serializers.indices) {
...
internal fun encodeMultiParams(
    ...
    values: Array<Any?>,
): ByteArray = factory.encode(ArrayBackedSerializer(descriptor, serializers), values)
...
internal fun decodeMultiParams(
    ...
    serializers: Array<KSerializer<*>>,
    payload: ByteArray,
): Array<Any?> = factory.decode(ArrayBackedSerializer(descriptor, serializers), payload)
...
@PublishedApi
@Suppress("UNCHECKED_CAST")
internal inline fun <reified T> getMultiParam(args: Array<Any?>, index: Int): T = args[index] as T
