...
[![Wasmtime](https://img.shields.io/badge/Wasmtime-45.0.3-5C9BD6?style=flat-square)](https://wasmtime.dev)
...
**双路径运行时架构。** 原生目标（Android、iOS、macOS、Linux、Windows）通过 Zig 0.15.1 编译的 `wasmline-core` 原生桥接层，以 JNI 或 Kotlin/Native C Interop 方式调用 **Wasmtime v45.0.3 (C-API)** 执行插件。Web 目标（Kotlin/JS、Kotlin/WasmJS）则通过浏览器原生 `WebAssembly.Module` / `WebAssembly.Instance` 容器以及内联 JavaScript 运行时执行插件——浏览器执行路径中不包含 Wasmtime。
...
|-------------------|---------------------|-----------------------------|-------------------------------|-----------------------|---------------------------|
| Android           | `arm64-v8a`         | Wasmtime v45.0.3 C-API      | JNI（Zig 0.15.1 编译）        | `.cwasm` / `.pwasm`   | `wasmline-core` Session   |
| iOS               | `arm64`             | Wasmtime v45.0.3 C-API      | C Interop（`.def` cinterop）  | `.pwasm`              | `wasmline-core` Session   |
| macOS             | `arm64`             | Wasmtime v45.0.3 C-API      | JNI（Zig 0.15.1 编译）        | `.cwasm` / `.pwasm`   | `wasmline-core` Session   |
| Linux             | `x86_64`            | Wasmtime v45.0.3 C-API      | JNI（Zig 0.15.1 编译）        | `.cwasm` / `.pwasm`   | `wasmline-core` Session   |
| Windows           | `x86_64`            | Wasmtime v45.0.3 C-API      | JNI（Zig 0.15.1 编译）        | `.cwasm` / `.pwasm`   | `wasmline-core` Session   |
...
./gradlew :wasmline-cli:run --args="build \
  -i plugin.wasm \
  -wt build/wasmline/wasmtime/wasmtime-v45.0.3-aarch64-macos \
