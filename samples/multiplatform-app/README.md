...
| Gradle module | Directory | Runtime contract |
|---|---|---|
| `sample-plugin` | `plugins/kotlin-service` | `CORE_WASM + WASMLINE_SERVICE` |
| `sample-raw-export-plugin` | `plugins/raw-export` | `CORE_WASM + RAW_EXPORT` |
| `sample-component-plugin` | `plugins/kotlin-component-service` | `COMPONENT_MODEL + WASMLINE_SERVICE` |
| `sample-component-export-plugin` | `plugins/kotlin-component-export` | `COMPONENT_MODEL + COMPONENT_EXPORT` |
| `sample-component-fixture` | `plugins/component-package` | Packages an external `wasmline:service` Component |

Gradle module names remain stable; `settings.gradle.kts` maps them to their
directories. C, C++, and Rust guest sources live in `plugins/c-component-service`,
`plugins/cpp-component-service`, and `plugins/rust-component-service`. Shared
CMake configuration lives in `plugins/build-support`. The plain WAT alternative
is in `plugins/raw-export/wat`; the Gradle Raw Export module compiles Kotlin.
These are parts of this application example, not separate runnable samples.
...
bash plugins/c-component-service/build.sh
```
...
WASMLINE_SAMPLE_COMPONENT_FIXTURE="$PWD/plugins/component-package/build/wasmline/output/crow.wasmline.component.fixture-1.0.0/manifest.wlm" \
  ./gradlew :sample-apps:multiplatform:desktopApp:run
```
...
the copied C Component with `plugins/cpp-component-service/build/plugin.component.wasm` for the C++
