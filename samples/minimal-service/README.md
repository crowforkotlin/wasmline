...
The host calls `GreetingService.greet("Wasmline")` and displays `Hello, Wasmline!`.
...
```bash
./gradlew -p samples/minimal-service :webApp:jsBrowserDevelopmentRun
./gradlew -p samples/minimal-service :webApp:wasmJsBrowserDevelopmentRun
```
