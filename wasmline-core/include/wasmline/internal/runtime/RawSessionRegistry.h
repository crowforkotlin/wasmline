...
namespace wasmline {
    ...
    public:
        ...
        /** Reads bytes from an explicit raw session into caller-owned storage. */
        InvocationResult readMemory(const std::string& sessionKey, uint64_t offset, uint8_t* destination, uint64_t length);
